					...
					implode(
						...
						array_filter(
							array(
								...
								pen_class_lists( 'date_updated_display_override', $content_id ),
							)
						)
					),
					sprintf(
						/* Translators: Update date. */
						esc_html_x( 'Updated on %s', 'Date', 'pen' ),
						sprintf(
							'<a href="%1$s" rel="bookmark">%2$s</a>',
							esc_url( $permalink ),
							sprintf(
...
if ( ! function_exists( 'pen_html_preview_enhance_navigation' ) ) {
	...
	function pen_html_preview_enhance_navigation() {
		...
		$url_preview = html_entity_decode( pen_filter_input( 'SERVER', 'REQUEST_URI' ) );
		...
		$links['font_groups'] = array(
			'url'  => $url_preview,
			'text' => __( 'Font Groups', 'pen' ),
		);

		$preset_font_current = (int) str_replace( 'preset_', '', pen_preset_get( 'font' ) );
