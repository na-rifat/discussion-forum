...
if ( ! function_exists( 'pen_pingback' ) ) {
	...
	function pen_pingback() {
		...
		if ( pen_is_singular() && $content_id && pings_open( $content_id ) ) {
	...
	function pen_body_classes( $classes ) {
		...
		if ( PEN_THEME_PREVIEW ) {
			...
			if ( 'plain' === pen_filter_input( 'GET', 'pen_preview_layout' ) ) {
				...
				if ( ! empty( $classes['invisible-sidebar-right'] ) ) {
					unset( $classes['invisible-sidebar-right'] );
				}
			}
			if ( ! empty( $classes['invisible-sidebar-bottom'] ) ) {
				unset( $classes['invisible-sidebar-bottom'] );
			}
			if ( ! empty( $classes['invisible-sidebar-left'] ) ) {
		...
		$classes[] = 'pen_navigation_mobile_' . ( 'never' !== pen_option_get( 'navigation_mobile_display' ) ? 'show' : 'hide' );
...
if ( ! function_exists( 'pen_class_lists' ) ) {
	 ...
	 * @return string
	 */
	function pen_class_lists( $option, $content_id = null ) {
...
if ( ! function_exists( 'pen_animations_delay' ) ) {
	...
	function pen_animations_delay() {
		...
		$text_seconds = __( '%d seconds', 'pen' );
		return array(
			0 => __( 'None', 'pen' ),
			1 => __( '1 second', 'pen' ),
			2 => sprintf( $text_seconds, 2 ),
			3 => sprintf( $text_seconds, 3 ),
			4 => sprintf( $text_seconds, 4 ),
			5 => sprintf( $text_seconds, 5 ),
		);
	}
}
...
if ( ! function_exists( 'pen_remind_rate_review' ) ) {
	...
	function pen_remind_rate_review() {
		...
		if ( $date_activated && ( time() - $date_activated ) > ( 2 * MONTH_IN_SECONDS ) ) {
			return true;
		} else {
			$date_downloaded = (int) filemtime( get_template_directory() );
			if ( $date_downloaded && ( time() - $date_downloaded ) > ( 3 * MONTH_IN_SECONDS ) ) {
				return true;
			}
		}
		return false;
