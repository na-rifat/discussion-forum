...
class bbPressModToolsPlugin_bbPress extends bbPressModToolsPlugin {
	...
	public function moderate_content( $content, $post_id ) {
		...
		if ( 'pending' == $post->post_status ) {
			...
			$notice = '<div class="bbp-mt-template-notice">' . __( 'This post is awaiting moderation.', $this->plugin_slug ) . '</div>';
	...
	public function user_admin_links() {
		...
		if ( $this->user_can_moderate( get_current_user_id() ) ) {
			...
			if ( isset( $action ) ) {
				...
				echo '<li><a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( ucfirst( $action ) . ' User', $this->plugin_slug ) . '</a></li>';
			}
			...
			if ( ! in_array( $role, array( 'Keymaster', 'Moderator', 'Senior Moderator' ) ) && $author_id > 0 && ! get_user_meta( $author_id, '_bbp_moderation_flagged', true ) ) {
				...
				$lockdown_action_text = __( 'Add User Moderation Flag',  $this->plugin_slug );
			} elseif ( get_user_meta( $author_id, '_bbp_moderation_flagged', true ) ) {
				$lockdown_action = 'remove_moderation_flag';
				$lockdown_action_text = __( 'Remove User Moderation Flag',  $this->plugin_slug );
			}
			...
			if ( isset( $lockdown_action ) ) {
				...
				$nonce_url = wp_nonce_url( $url, 'moderator_action', $this->plugin_slug . '-wp_nonce' );
				echo '<li><a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . $lockdown_action_text . '</a></li>';
			}
	...
	public function pending_title( $title, $post_id ) {
		...
		if ( 'pending' == $post->post_status ) {
			return $title . ' ('. __( 'Awaiting moderation', $this->plugin_slug ) . ')';
		}
		...
		if ( $this->user_can_moderate() ) {
			...
			if ( $child_count > 1 ) {
				return $title . ' ('. $child_count . ' ' . __( 'Replies awaiting moderation', $this->plugin_slug ) . ')';
			} else if ( $child_count == 1 ) {
				return $title . ' ('. $child_count . ' ' . __( 'Reply awaiting moderation', $this->plugin_slug ) . ')';
			}
		}
	...
	public function add_moderation_links( $links, $post_id ) {
		...
		if ( $this->user_can_moderate() ) {
			...
			if ( 'spam' != $post_status && 'pending' == $post_status && ! $is_pre_bbpress_2_6 ) {
				...
				$nonce_url = wp_nonce_url( $url, 'moderator_action', $this->plugin_slug . '-wp_nonce' );
				array_push( $links, '<a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( 'Approve', $this->plugin_slug ) . '</a>' );
				...
				$nonce_url = wp_nonce_url( $url, 'moderator_action', $this->plugin_slug . '-wp_nonce' );
				array_push( $links, '<a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( 'Unapprove', $this->plugin_slug ) . '</a>' );
