<?php
class bbPressModToolsPlugin_Report extends bbPressModToolsPlugin {
	...
	public function add_report_link( $links, $post_id ) {
		...
		if ( is_user_logged_in() && get_option( '_bbp_report_post' ) ) {
			...
			array_push( $links,
				...
				<span class="bbp-report-type">
					...
					<option>' . __( 'Report reason', $this->plugin_slug ) . '</option>
					<option>' . __( 'Spam', $this->plugin_slug ) . '</option>
					<option>' . __( 'Advertising', $this->plugin_slug ) . '</option>
					<option>' . __( 'Harassment', $this->plugin_slug ) . '</option>
					<option>' . __( 'Inappropriate content', $this->plugin_slug ) . '</option>
					</select>
				</span>'
			);
		...
		if ( $this->is_reported( $post_id ) && $this->user_can_manage_reports() ) {
			...
			array_push( $links, '<a href="' . $nonce_url . '" class="bbp-reply-edit-link">' . __( 'Clear reports', $this->plugin_slug ) . '</a>' );
	...
	*/
	public function handle_report_post() {
		...
		_e( 'Your report will be reviewed by our moderation team.', $this->plugin_slug );
...
bbPressModToolsPlugin_Report::init();
